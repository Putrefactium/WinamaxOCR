...
start_pid_handle_threads(exe_name) # Initialiser les threads pour commencer le traitement en arrière-plan
...
while not stop_script.is_set():
        ...
        if title == window_title:
            ...
            if win32gui.IsWindowVisible(hwnd):
                ...
                if x == -32000 and y == -32000:
                    ...
                    # Pour vérifier la présence du texte que tous les X secondes définies par la variable search_interval afin de ne pas ralentir avec trop d'OCR
                    ...
                    if current_timestamp - last_search_time >= search_interval:
                        ...
                        if img:
                            ...
                            found, elapsed_time = search_text_in_image(img, search_text)
                            
                            if found:
                                print_duration(search_start_time-elapsed_time, "Texte Sta trouvé")
                        ...
                        if found:
                            search_start_time = time.time()
                            print_duration(search_start_time, "Dessiné le bouton")
                            button_x, button_y = calculate_percentage_and_position(x, y, width)
                            show_button((button_x, button_y), hwnd)
            else:
                print(f"[{current_time}] HWND : {hwnd}, Titre : {title}, Visible : Non (Réduite)")
    ...
    if not found_window:
        ...
        # Redémarrer les threads pour obtenir des informations mises à jour
        if current_timestamp - last_search_time >= (search_interval):
        ...
        print("Les threads sont toujours en cours d'exécution.")
